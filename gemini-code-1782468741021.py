...
def auto_discover_schema(dataframe):
    ...
    if not schema['target_col']:
        schema['target_col'] = cols[-1] # Fallback to final matrix column
    ...
    for c in cols:
        ...
        if any(k in c_upper for k in ['AGE', 'BIRTH']):
            schema['age_col'] = c
        elif any(k in c_upper for k in ['INCOME', 'SALARY', 'EARNING', 'RATE', 'MONTHLYINCOME']):
            schema['income_col'] = c
        elif any(k in c_upper for k in ['ZONE', 'DEPARTMENT', 'TEAM', 'BRANCH', 'OFFICE', 'LOCATION', 'STATE']):
            schema['group_col'] = c
...
def universal_cleanse_pipeline(file_source):
    ...
    for col in df_clean.columns:
        if df_clean[col].dtype == 'object':
            # Check if strings are convertable to finances containing commas/symbols
    ...
    # Re-evaluate discovered schema shapes post-cleaning
...
# Risk anchor targets are identified as the less frequent outcome category (e.g., ATTRITION, REJECTION, CHURN)
...
with tab_modeling:
    ...
    if st.button("🚀 Train Model Classifiers Across Active Data Schema", type="primary"):
        with st.spinner("Processing feature extraction pipelines, encoding arrays, and optimizing hyperparameters..."):
            ...
            if 'HAS_REPORTED_INCOME' in ml_df.columns: ml_df = ml_df.drop(columns=['HAS_REPORTED_INCOME'])
            
            # Label encode target strings cleanly
            le_tgt = LabelEncoder()
            ...
            for name, clf in classifiers.items():
                ...
                probs = clf.predict_proba(X_test_proc)[:, 1] if hasattr(clf, "predict_proba") else clf.decision_function(X_test_proc)
...
with tab_findings:
    ...
    # Live algorithmic aggregations compiled directly from the generic table shape array
    live_age_ct = pd.crosstab(df['DYNAMIC_QUANTILE_BINS'], df[target_var], normalize='index') * 100 if 'DYNAMIC_QUANTILE_BINS' in df.columns else pd.DataFrame()
...
# =======================================================================================
# VI. INTEGRATED REAL-TIME ML CHATBOX PREDICTOR ASSISTANT
# =======================================================================================
st.sidebar.markdown("---")
...
if user_query:
    if st.session_state.trained_champion_model is None:
        ...
        # Programmatic analysis triggered by text prompts
        ...
        # Locate mapped indexes matching highest failure parameters
        ...
        if len(risk_indices) > 0:
            ...
            # Display interactive subset summary table directly inside chat workspace interface
            st.sidebar.markdown(f"**Top High-Risk Accounts List (`{risk_anchor_target}` Track):**")
            # Pull identifier primary key dynamically if existing
            ...
            # Automatically project corresponding risk distribution layouts
            st.markdown("---")
